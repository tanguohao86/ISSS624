...
```{r}
#| echo: false # auto is not going to run this code chunk
...
The previous comparison shows the numbers of radios whereas this comparison shows the penetration rate of the radio. there are some region with lower number of households but there is higher number of radio per 1000 household (penetration rate).
...
In the code chunk below, *normalize()* of [*heatmaply*](https://cran.r-project.org/web/packages/heatmaply/) package is used to standardize the clustering variables by using Min-Max method. The *summary()* is then used to display the summary statistics of the standardized clustering variables.
...
Z-score standardization can be performed easily by using [*scale()*](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/scale) of Base R. The code chunk below will be used to standardize the clustering variables by using Z-score method.
...
The code chunk below uses [*as_Spatial()*](https://r-spatial.github.io/sf/reference/coerce-methods.html) of **sf** package to convert *shan_sf* into a SpatialPolygonDataFrame called *shan_sp*. #Spatially Constrained Clustering: SKATER approach
