...
[Moran I statistic of 0.300749970, there is some clustering of similar value]{.underline}

[p-value = 1.095e-06 \< 0.05, hence result is significant]{.underline}
...
the result of statistic = 0.30075 ([some clustering of similar value)]{.underline} is significant as p-value = 0.001. This confirms the findings of Maron's I test.
...
```{r}
ggplot(df, aes(bperm$res)) + geom_histogram(
  mapping = NULL,
  data = df,
  stat = "bin",
  position = "stack",
...
Positive spatial correlation as Geary C statistic (0.6907223) is approaching zero.
...
Positive spatial correlation as Geary C statistic (0.6907223) is significant as p-value = 0.001.

#### 4.4.5.3 Visualizing the Monte Carlo Geary's C
...
values for 4 and 6 are not statistically significant.
...
ANS: right plot labelled red areas have higher than average GDPPC than their neighbors. This is confirmed that the GDPPC plot.
...
ANS: if the p-value is statistically significant, positive z-score means spatial distribution of high values in the dataset is more spatially clustered than would be expected if underlying spatial processes were truly random while negative z-score means spatial distribution of low values in the dataset is more spatially clustered than would be expected if underlying spatial processes were truly random. This can all be confirmed by the GDPPC plot.
...
I noticed that more areas with 0-20,000 GDPPC are indicated as high local GI.
